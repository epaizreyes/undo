...
UndoHistory::UndoHistory(std::shared_ptr<UndoHistoryDelegate> delegate)
    : m_delegate(delegate) , m_first(nullptr) , m_last(nullptr)
    , m_cur(nullptr)
{}
...
{
    std::shared_ptr<UndoState> state = m_last;
    ...
    for (std::shared_ptr<UndoState> prev = nullptr;
        state && state != m_cur;
        state = prev) {
        prev = state->m_prev;
        deleteState(state);
...
std::shared_ptr<const UndoState> UndoHistory::findCommonParent(const std::shared_ptr<const UndoState>& a,
                                                               const std::shared_ptr<const UndoState>& b)
{
  std::shared_ptr<const UndoState> pA = a;
  std::shared_ptr<const UndoState> pB = b;
